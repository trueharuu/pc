...
function render(v, m = should_mirror) {
    if (v === '') v = 'v115@vhAAgh';
    const img = document.createElement('img');
    img.src = `https://qv.rqft.workers.dev/fumen?data=${encodeURIComponent(v)}&lcs=false&scale=3&mirror=${m}`;
    img.onclick = () => {
        // write the fumen string to clipboard
...
title.textContent = `${sort(data['@pc'])} ${data['@percent'] * 100}% ${sort(data['@save'])}`;
document.head.appendChild(title);
const container = document.createElement('div');

const header = document.createElement('h1');
header.classList.add('title');
header.textContent = `${sort(data['@pc'])} ${data['@percent'] * 100}% ${sort(data['@save'])}`;
...
function determine_used_setup(piece1, piece2, piece3) {
    ...
    candidates.sort((a, b) => {
        ...
        const aDupes = a.replace(/\+/g, '').split('').some((c, i, arr) => arr.indexOf(c) !== i);
        const bDupes = b.replace(/\+/g, '').split('').some((c, i, arr) => arr.indexOf(c) !== i);
...
navtitle.textContent = `${data['@pc']} ${data['@percent'] * 100}% ${data['@save']}`;
